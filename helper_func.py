...
async def delete_file(messages: list, client, process, user_id: int = None):
    ...
    Persists the scheduled delete to MongoDB so it survives restarts.
    """
    if user_id:
        from database.database import is_premium
        if await is_premium(user_id):
    ...
    import time
    from database.database import save_pending_delete, remove_pending_delete

    # Persist to DB before sleeping so restart can recover
    msg_ids   = [msg.id for msg in messages]
    chat_id   = messages[0].chat.id if messages else None
    delete_at = time.time() + AUTO_DELETE_TIME

    if chat_id:
        await save_pending_delete(
            user_id=user_id or 0,
            message_ids=[{'chat_id': chat_id, 'msg_id': m} for m in msg_ids],
            delete_at=delete_at
        )
