...
async def open_support_callback(client, query):
    from database.database import open_chat_session, get_chat_session, get_bot_admins
    user_id = query.from_user.id
    session = await get_chat_session(user_id)

    if session:
        await query.answer("You already have an active session!", show_alert=True)
        return
    ...
    await query.message.edit_text(
        ...
        "<i>Use /closechat to end the session.</i>",
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("❌ Close Session", callback_data="close_my_chat")
        ]])
    )
        ...
        try:
            await client.send_message(
                ...
                text=(
                    ...
                    f"👤 <b>User:</b> <a href='tg://user?id={user_id}'>"
                    f"{query.from_user.first_name}</a> (<code>{user_id}</code>)\n\n"
                    f"<i>Tap below to start chatting</i>"
                ),
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton(
                        f"💬 Chat with {query.from_user.first_name}",
            ...
            pass
    await query.answer("Support session opened!")
