...
async def cfg_dbchannel_byforward(client, query: CallbackQuery):
    ...
    try:
        resp = await client.ask(
            query.from_user.id, "",
            filters=filters.forwarded, timeout=60
        )
        if resp.forward_from_chat:
            new_id = resp.forward_from_chat.id
            await set_setting('db_channel_id', new_id)
            # Save access hash
            if hasattr(resp.forward_from_chat, 'access_hash') and resp.forward_from_chat.access_hash:
                await set_setting('channel_access_hash', resp.forward_from_chat.access_hash)
            import config
            config.CHANNEL_ID = new_id
            await resp.reply(
                f"✅ <b>DB Channel updated!</b>\n\n"
                f"Channel: <b>{resp.forward_from_chat.title}</b>\n"
