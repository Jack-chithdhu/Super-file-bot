...
async def cfg_dbchannel_byforward(client, query: CallbackQuery):
    ...
    try:
        resp = await client.ask(
            query.from_user.id, "",
            filters=filters.all, timeout=60
        )
        # Check forward_origin (newer Pyrogram) or forward_from_chat (older)
        channel = None
        if hasattr(resp, 'forward_origin') and resp.forward_origin:
            if hasattr(resp.forward_origin, 'chat'):
                channel = resp.forward_origin.chat
        if not channel and resp.forward_from_chat:
            channel = resp.forward_from_chat
        ...
        if channel:
            ...
            await resp.reply("❌ Could not detect channel. Make sure you forward from a channel, not a user.")
    except asyncio.TimeoutError:
        await client.send_message(query.from_user.id, "❌ Timed out.")
    await show_main_settings(client, query.from_user.id)
