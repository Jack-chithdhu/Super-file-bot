...
# ─────────────────────────────────────────────────────────────────────────────
#  Relay: User → Admin
#  Any message from a user with an active session is forwarded to all admins
# ─────────────────────────────────────────────────────────────────────────────
...
async def relay_message(client: Client, message: Message):
    ...
    if user_id in all_admins:
        if not hasattr(client, 'admin_chat_targets'):
            client.admin_chat_targets = {}
        target_id = client.admin_chat_targets.get(user_id)
        if target_id:
            try:
    ...
    # ── User has active session — forward to admins ───────────────────────
    session = await get_chat_session(user_id)
    if session:
        relay_header = (
