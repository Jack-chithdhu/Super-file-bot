...
async def needs_captcha(user_id: int) -> bool:
    ...
    if user_id in verified_users:
        return False
    from database.database import present_user
...
async def captcha_callback(client: Client, query: CallbackQuery):
    ...
    start_text    = f"/start {original_text}" if original_text else "/start"

    # Simulate /start by sending it programmatically
    from pyrogram.types import Message as PyroMessage
    try:
        await client.send_message(chat_id=user_id, text=start_text)
    except Exception:
        pass
