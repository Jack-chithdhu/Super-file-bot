...
async def captcha_callback(client: Client, query: CallbackQuery):
    ...
    try:
        ...
        # Send the fake message, call start_command FIRST, then delete
        sent = await client.send_message(
            chat_id=user_id,
            text=fake_text,
            disable_notification=True
        )
        # Call start_command while message still exists so message.text is readable
        await start_command(client, sent)
        # Now safe to delete the helper message
        try:
            await sent.delete()
        except Exception:
            pass
